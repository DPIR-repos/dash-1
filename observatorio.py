...
####dataframe con la base de datos 
            with st.expander(f" **Base de datos para el código**: {', '.join(map(str, insumoCode)) if isinstance(insumoCode, list) else insumoCode} - {df_filtrado['Insumo Match'].iloc[0]}"):
                ...
                st.markdown(""" **Nota:** Esta base de datos incluye únicamente insumos con un score ≥ 0.8. 
                            Todas las estadísticas y métricas mostradas aplican este mismo criterio. 

                            Si deseas consultar insumos con score ≥ 0.7, dirígete a la sección 
        ...
        if len(variedad_select)>=1:
            inflacion_choice=st.sidebar.selectbox(
                    "Tipo de correcion por inflacion:",
        ...
        with tab2:
            ...
            if variedad:
                for idx, v in enumerate(variedad):
                    ...
                    if inflacion_choice is not None:
                        #----CORRECCION-POR INFLACION-----
                        anio_max=df_v['Anio Publicacion'].min() #Obtengo el minimo año que hay en la data
                        df_v_anio=df_v[df_v['Anio Publicacion']==anio_max].copy() #filtro con respecto al año minimo
                        month_max=df_v_anio['Mes Publicacion'].min() #Obtengo el mes minimo que hay en la variedad
...
#================================        
        ...
        formato_descarga = st.sidebar.radio(
            "📤 Descargar info. completa por código:",
